...
if [ -f "$CA_WORKING_DIRECTORY/$CA_KEY_FILE" ] && [ -f "$CA_WORKING_DIRECTORY/$CA_CERT_FILE" ]; then
  ...
  echo
  MSYS_NO_PATHCONV=1 openssl req -new -newkey rsa:4096 -days $VALIDITY_IN_DAYS -x509 \
  -subj "/CN=${CN}" \
  -keyout $CA_WORKING_DIRECTORY/$CA_KEY_FILE \
  -out $CA_WORKING_DIRECTORY/$CA_CERT_FILE \
  -nodes
...
while read -r KAFKA_HOST || [ -n "$KAFKA_HOST" ]; do
  ...
  echo $KAFKA_HOST
  echo $SUFFIX
  echo $KEY_STORE_FILE_NAME
  echo $KEYSTORE_WORKING_DIRECTORY
  ...
  echo "  -> File: $KEYSTORE_WORKING_DIRECTORY/$KEY_STORE_FILE_NAME"
  echo "  -> DNAME: $DNAME"

  [[ ! "$KEY_STORE_FILE_NAME" =~ \.jks$ ]] && echo "❌ Invalid filename generated: $KEY_STORE_FILE_NAME" && exit 1
  ...
  -dname $DNAME -ext SAN=DNS:$KAFKA_HOST,DNS:$KAFKA_HOST.kafka-network,DNS:localhost \
